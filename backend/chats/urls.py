...
from .views import (
    ...
    LeaveChatView, BlockUserView, UnblockUserView
)
...
urlpatterns = [
    ...
    path('<int:chat_id>/messages/<int:pk>/', MessageRetrieveUpdateDestroyView.as_view(), name='message-detail'),
    
    # Приглашения
    path('<int:chat_id>/invite/', InviteUserView.as_view(), name='chat-invite'),
    path('<int:chat_id>/invite/accept/', AcceptInviteView.as_view(), name='chat-invite-accept'),
    path('<int:chat_id>/invite/reject/', RejectInviteView.as_view(), name='chat-invite-reject'),
